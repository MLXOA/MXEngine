...
public class Model : IDisposable
{
    ...
    public Model(GL gl, Stream file, bool gamma = false)
    {
    ...
    private List<Tex2D> _texturesLoaded = new List<Tex2D>();
    ...
    private unsafe void LoadModel(Stream file)
    {
        using (var memoryStream = new MemoryStream())
        {
            file.CopyTo(memoryStream);
            fixed (byte* ptr = memoryStream.ToArray())
            {
                var scene = _assimp.ImportFileFromMemory(ptr, (uint)memoryStream.Length, (uint)PostProcessSteps.Triangulate, "");
    ...
    private unsafe Mesh ProcessMesh(AssimpMesh* mesh, Scene* scene)
    {
        ...
        List<Tex2D> textures = new List<Tex2D>();
    ...
    private unsafe List<Tex2D> LoadMaterialTextures(Material* mat, TextureType type, string typeName)
    {
        ...
        for (uint i = 0; i < textureCount; i++)
        {
            ...
            if (!skip)
            {
                var texture = new Tex2D(_gl, Directory, type);
                texture.Path = path;
